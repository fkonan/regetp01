...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Edit Dependencia', true), array('action'=>'edit', $dependencia['Dependencia']['id'])); ?> </li>
		<li><?php echo $html->link(__('Delete Dependencia', true), array('action'=>'delete', $dependencia['Dependencia']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $dependencia['Dependencia']['id'])); ?> </li>
...
<div class="related">
	...
	<?php if (!empty($dependencia['Instit'])):?>
	...
	<?php
		$i = 0;
		foreach ($dependencia['Instit'] as $instit):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
