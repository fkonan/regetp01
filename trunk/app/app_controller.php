...
class AppController extends Controller {
	...
	var $components = array('Acl', 'Session', 'Auth', 'Requesthandler','DebugKit.Toolbar');
	...
	function beforeFilter(){
		...
		$this->Auth->authError = 'Debe registrarse para acceder a esta p�gina';
		...
		$this->Auth->authError = 'Usted no tiene permisos para acceder a esta p�gina.';
                $this->Auth->planesMejoraError = 'Usted no tiene acceso a los planes de mejora de otra jurisdicci�n';
	...
	function isAuthorized2()
	{
	  switch ($this->Auth->user('role')):
		...
		case 'editor':
		  ...
		  case 'invitado':	
			...
			if ($this->name == 'HistorialCues' && $this->action == 'search') {$llAuth = true;}
                        ...
                        break;
		  default: die('El tipo de usuario no concuerda con ninguno setteado en el sistema y se abortar� la operaci�n');
		endswitch;
		...
		*/
		...
		/**
		 * Hacer que solo puedan modificar sus datos y contrase�a el usuario que es due�o de esos datos
		 */
	   if ($this->name == 'Users' && $this->action == 'cambiar_password' && $this->passedArgs[0] == $this->Auth->user('id')) {$llAuth = true;}
		...
		if ($llAuth == true) {
			...
			$this->Session->setFlash('No tiene permisos para acceder a esta opci�n.', true);
