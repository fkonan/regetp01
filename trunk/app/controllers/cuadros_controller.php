...
class CuadrosController extends AppController {
	...
	function total_instits_por_gestion_jurisdiccion() {
		$this->CustomQuery->sql = "
				(SELECT j.name as \"División politíco-territorial\" ,
                ...
                UNION ALL
                (
                 SELECT cast ('Universidades Nacionales' as varchar(40)) as \"División politíco-territorial\",
	                    sum(CASE WHEN (i.gestion_id = 1) THEN 1 ELSE 0 END )as \"Gestión Estatal\" ,
	                    sum(CASE WHEN (i.gestion_id = 2) THEN 1 ELSE 0 END )as \"Gestión Privada\" ,
                        count(*) as \"Total\"
                 FROM  instits i
                ...
                UNION ALL
                (
                 SELECT cast ( 'Total' as varchar(40)) as \"División politíco-territorial\" ,
                 sum(CASE WHEN (i.gestion_id = 1) THEN 1 ELSE 0 END ) as \"Gestión Estatal\" ,
                 sum(CASE WHEN (i.gestion_id = 2) THEN 1 ELSE 0 END )as \"Gestión Privada\",
                 count(*) as \"Total\"
                 FROM   instits i 
